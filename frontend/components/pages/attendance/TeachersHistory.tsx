...
const TeachersHistory: React.FC = () => {
    ...
    useEffect(() => {
        const fetchHistory = async () => {
            ...
            setError('');
            try {
                const response = await api.get(`/attendance/user/${selectedStaff}`);
    ...
    return (
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
            <h2 className="text-xl font-semibold text-gray-800 mb-4">Teacher Attendance History</h2>
            <div className="mb-6">
                <label className="block text-sm font-medium text-gray-700">Select Staff Member</label>
                <select value={selectedStaff} onChange={e => setSelectedStaff(e.target.value)} className="w-full md:w-1/2 p-2 border rounded mt-1">
                    <option value="">-- Select a Staff Member --</option>
                    {staff.map(s => <option key={s.userID} value={s.userID}>{s.name} {s.surname} ({s.userID})</option>)}
                </select>
            </div>
